...
const NavBar = ({categories}) => {
  ...
  return (
    <nav  className={`flex px-2 group justify-between z-50 border-b-2 border-gray-700 bg-transparent hover:text-black hover:bg-[#c6c3d6] transition-[background] duration-500 relative`}>
      <img src="/favicon.ico" alt="" />

      <div className='md:flex hidden gap-6 py-3 m-auto'>
      ...
      <div onMouseLeave={() => setActive(false)} className={`absolute bg-[#c6c3d6] ${active ? 'block' : 'hidden'} w-screen py-4 left-0 gap-10 flex top-[100%] flex-wrap justify-center mx-auto`}>
        {categories.map((category, id) => (
          <div key={id} className='flex flex-col'>
            ...
            {category.children.map((child, id) => (
              <a key={id} href={`/products/${child.slug}`} className='hover:text-[#853e95] hover:underline py-[4px] text-sm text-gray-800'>
                {child.name}
              </a>
            ))}
          </div>
        ...
        {categories.map((category) => category.children.map((child, id) => (
          child.assets.length > 0 && (
            <a key={id} href={`/products/${child.slug}`} className=' pt-4 text-sm text-gray-800'>
      ...
      <div className={`${mobileNav ? 'left-0' : '-left-[100%]'} overflow-y-scroll md:hidden transition-all duration-300 fixed uppercase font-light h-screen ease-in-out w-screen z-[100] hover:text-white text-white tracking-[0.2rem]`} >
        ...
        <div onClick={() => setMobileNav(false)} className='absolute top-0 right-0 w-2/12 h-screen bg-transparent' />
